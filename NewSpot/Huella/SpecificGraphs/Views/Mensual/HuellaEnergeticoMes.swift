...
struct HuellaEnergeticaMes: View {
    ...
    var body: some View {
        VStack {
            ...
            ZStack {
                if let highestEmissions = energeticoMes.max(by: {
                    ...
                    if graphType == .bar {
                        ...
                        if let barSelection, let selectedEmissions = energeticoMes.findEmissions(barSelection) {
            ...
            .task{
                ...
                for i in 0...5 {
                    ...
                    energeticoMes.append(.init(date: startofmonth!, emissions: num))
                }
                loading = false
            }
            ...
            Chart {
                ForEach(energeticoMes.sorted(by: { graphType == .bar ? false : $0.emissions > $1.emissions })) { emission in
                    if graphType == .bar {
                        BarMark(
                    ...
                    RuleMark(x: .value("Month", barSelection))
                        ...
                        .annotation(
                            ...
                            overflowResolution: .init(x: .fit, y: .disabled)) {
                                if let emissions = energeticoMes.findEmissions(barSelection) {
    ...
    func findEmission(_ rangeValue: Double) {
        ...
        let convertedArray = energeticoMes
